...
const { CompositionStage, useComposition, Shot, Captions, Easing, animate, interpolate, clamp } = window;
...
const BLACK = '#080808';
const GRAY = '#888888';
const NOTE = 'gate code 4471. delete this.';
const LINK = 'https://app.nosus.foo/#/burn/7f3a9c2e';
const LINK_SHORT = 'app.nosus.foo/#/burn/7f3a9c2e';
  ...
  return (
    <div style={center}>
      ...
      <div style={{
        ...
        <div style={{ font: `400 10px ${MONO}`, lineHeight: 1.5, color: '#fff', textAlign: 'center' }}>
          {typed}<span style={{
...
function Working({ T, C, from }) {
  ...
  return (
    <div style={{ ...center, gap: 16, opacity: inn, transform: `scale(${0.94 + 0.06 * inn})` }}>
...
function DoneCard({ T, C }) {
  ...
  return (
    <div style={{ ...center, gap: 8, padding: '0 12px' }}>
    ...
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: out }}>
      <div style={{ transform: `scale(${L.end})`, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
        <div style={{ opacity: logo, transform: `translateY(${(1 - logo) * 10}px)`, whiteSpace: 'nowrap' }}>
          <window.NoSus.NoSusLogo sizeClass="text-6xl" />
        </div>
        <div style={{
...
/* ── audio bed ───────────────────────────────────────────────
   Clips live in audio/ and are fired off the authored clock, so
   trims and speed changes on the timeline carry them along. */
const CLIPS = {
  typing: 'audio/typing.wav',
  click: 'audio/click.wav',
  latch: 'audio/latch.wav',
  tick: 'audio/tick.wav',
...
function AudioBed({ C, T, on, total }) {
  const p = React.useRef(null);
  const prev = React.useRef(-1);
  if (!p.current) {
    p.current = {};
    for (const k in CLIPS) { const a = new Audio(CLIPS[k]); a.preload = 'auto'; p.current[k] = a; }
    p.current.tick.loop = true;
    p.current.tick.volume = 0.5;
    ['click', 'latch', 'drop'].forEach((k) => { p.current[k].volume = 0.5; });
    p.current.typing.volume = 0.4;
    p.current.erase.volume = 0.55;
  }
  // one unlock on the first gesture anywhere, so the first cue isn't swallowed
  React.useEffect(() => {
    const unlock = () => {
      Object.values(p.current).forEach((a) => {
  ...
  React.useEffect(() => {
    const a = p.current;
    if (!on) { Object.values(a).forEach((x) => x.pause()); prev.current = T; return; }
    const t0 = prev.current, t1 = T;
    prev.current = T;
    const advanced = t1 > t0 && t1 - t0 < 1.2;
    if (advanced) {
      [['typing', 0.2], ['click', C.Reveal + 2.0], ['latch', C.Ready + 0.15],
       ['click', C.Ready + 3.1], ['erase', C.Open + 3.6], ['drop', C.File + 0.55]]
        .forEach(([k, at]) => { if (t0 < at && t1 >= at) { try { a[k].currentTime = 0; a[k].play(); } catch (e) {} } });
    }
    // the voice is one long take — keep it locked to the clock instead of fired once
    const voAt = C.Reveal + 0.1;
    if (T >= voAt && T < total - 0.05) {
      const want = T - voAt;
      if (a.vo.paused) { a.vo.currentTime = Math.max(0, Math.min(want, (a.vo.duration || 99) - 0.05)); a.vo.play().catch(() => {}); }
      else if (Math.abs(a.vo.currentTime - want) > 0.5) a.vo.currentTime = want;
    } else if (!a.vo.paused) a.vo.pause();
    const ticking = (T >= C.Encrypt && T < C.Ready) || (T >= C.File + 1.4 && T < C.End);
    if (ticking && a.tick.paused) a.tick.play().catch(() => {});
    else if (!ticking && !a.tick.paused) a.tick.pause();
  }, [T, on]);
  React.useEffect(() => () => Object.values(p.current).forEach((x) => x.pause()), []);
  return null;
}

/* ── the piece ──────────────────────────────────────────────── */
function Piece({ L, supers, sound }) {
  ...
  return (
    <div data-screen-label={`t=${Math.floor(T)}s`} style={{ position: 'absolute', inset: 0, background: BLACK, overflow: 'hidden' }}>
      {/* establishing frame: the live BurnTool */}
      {/* the circle, one element across beats 1–4 and 7 */}
      <div style={{
        ...
        {/* cursor pressing BURN NOTE, in circle space */}
        <div style={{
      ...
      <AudioBed C={C} T={T} on={sound} total={authoredTotal} />
      {supers && (
        <Captions
          items={[
...
const TWEAK_DEFAULTS = Object.assign({ motionEditor: true, supers: true, sound: true }, window.OM_TWEAKS || {});

function BurnAd(props) {
  ...
  return (
    <React.Fragment>
      <CompositionStage width={w} height={h} scenes={props.scenes} playback={props.playback} bg={BLACK}>
        <Piece L={L} supers={t.supers !== false} sound={t.sound !== false} />
      </CompositionStage>
      <TweaksPanel>
        <TweakSection label="Ad" />
        <TweakToggle label="Supers" value={t.supers !== false} onChange={(v) => setTweak('supers', v)} />
        <TweakToggle label="Sound" value={t.sound === true} onChange={(v) => setTweak('sound', v)} />
