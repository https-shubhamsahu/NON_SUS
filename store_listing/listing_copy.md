...
Do not say zero-knowledge, screenshot-proof, or that a link share keeps the key off the server: every single note or single file share also stores its key for the 2-digit code.
...
Send self-destructing notes and files. They are encrypted on your device before upload, and the decryption key travels in the link. So a recipient can open a single note or file with a 2-digit code instead, a copy of its key is also stored on our server until the code is used or expires (20 minutes by default). Multi-file shares don't use codes, so their keys stay in the link. One view or download, and the file is permanently deleted. Recipients don't need an account.
